...
export function purchasePrintInput(data: DbShape, inv: PurchaseInvoice): InvoicePrintInput {
  ...
  return {
    ...
    customerPhone: [supplier?.phone, supplier?.phone2].filter(Boolean).join(" / ") || undefined,
    customerCode: supplier?.code,
    customerAddress: supplier?.address,
    customerCity: supplier?.city,
    customerEmail: supplier?.email,
    customerContact: supplier?.contactPerson,
    customerCommercialNo: supplier?.commercialNo,
    customerTaxNo: supplier?.taxNo,
    customerTaxOffice: supplier?.taxOffice,
    customerActivity: supplier?.activity,
